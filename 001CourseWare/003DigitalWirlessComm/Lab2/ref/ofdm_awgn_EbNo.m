...
for n = 1 : 20
    for i = 1 : times
        ...
        snrdB =convertSNR(EbNo,"ebno","snr",BitsPerSymbol=bps); % SNR = Eb / N0 + 10log10((Nbps×R) / Nsps)
    ...
    bitErrorRate(n) = errors / length(overAllData(:));
    disp(['Bit Error Rate: ', num2str(bitErrorRate(n))]);
end

plot(EbNo, bitErrorRate);
xlabel('Eb/No (dB)')
